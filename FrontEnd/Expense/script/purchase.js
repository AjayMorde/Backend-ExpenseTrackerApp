...
function showLeaderboard() {
    ...
    inputElement.onclick = async () => {
        ...
        try {
            const response = await axios.get('http://34.235.120.189:3000/premiumuser/showLeaderBoard', {
...
document.getElementById('rzp-button1').onclick = async function (e) {
    const token = localStorage.getItem('token')
    const response = await axios.get('http://34.235.120.189:3000/purchase/premiummembership', { headers: { "Authorization": token } });
    ...
    var options =
    {
       ...
       //handling a success payment
        "handler": async function (response) {
            const res = await axios.post('http://34.235.120.189:3000/purchase/updatetransactionstatus', {
